 ...
 if($total != 0){
    ...
    while($result =mysqli_fetch_assoc($data))
    {
        ...
        echo "<tr>
                ...
                <td><a href='update_design.php?id=$result[id]&na=$result[name]&ph=$result[phone]&ci=$result[city]&st=$result[state]&zi=$result[zip]&fr=$result[from]&de=$result[destination]&de=$result[departuredate]&re=$result[returndate]&em=$result[emailid]&al=$result[alternateno]&ge=$result[gender]&ho=$result[holdername]&mo=$result[modeofpayment]&ba=$result[bankname]'>Update</a></td>
